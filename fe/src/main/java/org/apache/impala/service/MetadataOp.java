...
public class MetadataOp {
  ...
  private static void initialzeResultSetSchemas() {
    ...
    GET_CROSS_REFERENCE_MD.addToColumns(
        new TColumn("KEQ_SEQ", Type.INT.toThrift()));
    GET_CROSS_REFERENCE_MD.addToColumns(
        new TColumn("UPDATE_RULE", Type.STRING.toThrift()));
    GET_CROSS_REFERENCE_MD.addToColumns(
        new TColumn("DELETE_RULE", Type.STRING.toThrift()));
    GET_CROSS_REFERENCE_MD.addToColumns(
        new TColumn("FK_NAME", Type.STRING.toThrift()));
    GET_CROSS_REFERENCE_MD.addToColumns(
        new TColumn("PK_NAME", Type.STRING.toThrift()));
    GET_CROSS_REFERENCE_MD.addToColumns(
        new TColumn("DEFERRABILITY", Type.STRING.toThrift()));
  }
   ...
   * the wait timeout has been reached.
   */
  public static TResultSet getPrimaryKeys(Frontend fe, String catalogName,
      String schemaName, String tableName, User user) throws ImpalaException {
    TResultSet result = createEmptyResultSet(GET_PRIMARY_KEYS_MD);
    // Get the list of schemas, tables that satisfy the search conditions.
    PatternMatcher schemaMatcher = PatternMatcher.createJdbcPatternMatcher(schemaName);
    PatternMatcher tableMatcher = PatternMatcher.createJdbcPatternMatcher(tableName);
    DbsMetadata dbsMetadata = getDbsMetadata(fe, catalogName, schemaMatcher,
        tableMatcher, PatternMatcher.MATCHER_MATCH_ALL, PatternMatcher.MATCHER_MATCH_NONE
        , user);
    ...
    if (LOG.isTraceEnabled()) LOG.trace("Returning " + result.rows.size() + " primary "
   ...
   */
  public static TResultSet getCrossReference(Frontend fe, String parentCatalogName,
      String parentSchemaName, String parentTableName,
      String foreignCatalogName, String foreignSchemaName,
      String foreignTableName, User user) throws ImpalaException {
    ...
    for (int i = 0; i < dbsMetadata.dbs.size(); ++i) {
      for (int j = 0; j < dbsMetadata.tableNames.get(i).size(); ++j) {
        ...
        // object. For this metadata we filter out the results based on the request and
    ...
    if (LOG.isTraceEnabled()) LOG.trace("Returning " + result.rows.size() + " foreign "
        + "keys for table " + foreignTableName);
    return result;
  }

  /**
   * Helper to return only the foreign keys for primary key tables requested in the FK
   * request.
   */
  private static List<SQLForeignKey> filterForeignKeys(List<SQLForeignKey> dbForeignKeys,
      String pkSchemaName, String pkTableName, String fkSchemaName, String fkTableName) {
    ...
    for (SQLForeignKey fk : dbForeignKeys) {
      if (fk.getPktable_db().equals(pkSchemaName) &&
          ...
          fk.getFktable_db().equals(fkSchemaName) &&
          fk.getFktable_name().equals(fkTableName)) {
        foreignKeys.add(fk);
      }
    }
    return foreignKeys;
